...
const AppFooter = () => {
  ...
  const { PROXY_ENABLED, EXTENSIONS_ENABLED } = store.settings;
  ...
  const onChange = (e: RadioChangeEvent) => {
    ...
    if (value === 'PROXY_ENABLED') {
      dispatch((state) => {
        ...
        state.settings.EXTENSIONS_ENABLED = false;
      });
    } else if (value === 'EXTENSIONS_ENABLED') {
      dispatch((state) => {
        state.settings.EXTENSIONS_ENABLED = true;
        state.settings.PROXY_ENABLED = false;
      });
    } else if (value === 'BROWSER_ENABLED') {
      dispatch((state) => {
        state.settings.EXTENSIONS_ENABLED = false;
        state.settings.PROXY_ENABLED = false;
      });
